package mike.cutil.client.util;
...
public class HudUtil {
    ...
    public static Vector4f project3Dto2D(Vec3d pos3D, Matrix4f modelViewMatrix, Matrix4f projectionMatrix) {
        Vec3d in3D = MinecraftClient.getInstance().gameRenderer.getCamera().getCameraPos().negate().add(pos3D);
        Window wnd = MinecraftClient.getInstance().getWindow();

        Quaternionf quaternion = new Quaternionf(in3D.x, in3D.y, in3D.z, 1.0f);
